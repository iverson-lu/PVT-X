...
- Standalone TestCase run: Direct execution of a Test Case outside any Suite execution graph.
...
- A standalone TestCase run MUST NOT belong to any Suite execution graph and MUST NOT use node-level inputs.
...
Example (repository organization only; Suites are independent of Plans):
```
TestSuites/
  Thermal/
    suite.manifest.json
...
- For Suite manifests, Test Case refs MUST resolve within ResolvedTestCaseRoot.
- For Plan manifests, Suite refs MUST resolve within ResolvedTestSuiteRoot.
- Engine MUST normalize resolved target paths before containment checks.
- After normalization, out-of-root targets MUST be rejected.
...
RunRequest SHOULD primarily target a Test Suite but MUST support standalone TestCase run.

Suite-targeted schema:
```
{
...
- For suite-targeted RunRequest, nodeOverrides keys MUST match nodeId values in the target Suite.
- For suite-targeted RunRequest, nodeOverrides MAY be omitted.
...
Standalone TestCase example:
```json
{
  "testCase": "CpuStress@2.0.0",
  "caseInputs": { "DurationSec": 30, "Mode": "A" }
}
```
...
- In each suite iteration, each node produces exactly one TestCase run.
- Over multiple iterations, the same node MAY produce multiple TestCase runs (one per iteration).
- Suite runs aggregate node results.
- Suite is the smallest reusable execution template.
- A standalone TestCase run executes a single Test Case outside any Suite execution graph.
