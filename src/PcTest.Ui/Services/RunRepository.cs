...
public sealed class RunRepository : IRunRepository
{
    ...
    public async Task<IReadOnlyList<RunIndexEntryViewModel>> GetRunIndexAsync(
        RunFilter filter,
        CancellationToken cancellationToken = default)
    {
        var runs = await GetRunsAsync(filter, cancellationToken);
        var viewModels = new List<RunIndexEntryViewModel>();

        foreach (var run in runs)
        {
            viewModels.Add(new RunIndexEntryViewModel
            {
                RunId = run.RunId,
                RunType = run.RunType,
                NodeId = run.NodeId,
