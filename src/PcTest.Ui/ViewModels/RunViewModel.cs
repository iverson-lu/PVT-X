...
public partial class RunViewModel : ViewModelBase
{
    ...
    public bool ShowRepeatIndicator => RunType == RunType.TestSuite && RepeatCount > 1;
    public string RepeatIterationBadgeText => RunType == RunType.TestSuite && RepeatCount > 1
        ? $"Repeat {RepeatCount}x · Iteration {CurrentIterationIndex}/{RepeatCount}"
        : string.Empty;
    public bool ShowIterationGroups => RunType == RunType.TestSuite && RepeatCount > 1;
    ...
    private void UpdateFlatNodes(RunExecutionState state)
    {
        var orderedNodes = state.Nodes
            .OrderBy(node => node.IterationIndex)
            .ThenBy(node => node.SequenceIndex)
            .ToList();

        for (int i = 0; i < orderedNodes.Count; i++)
        {
            var nodeState = orderedNodes[i];
            var existingVm = GetOrCreateNodeViewModel(nodeState);
        ...
        for (int i = Nodes.Count - 1; i >= orderedNodes.Count; i--)
        {
            Nodes.RemoveAt(i);
        }
    }
